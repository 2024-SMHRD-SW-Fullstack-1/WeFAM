...
const HouseWork = () => {
  ...
  const addTask = () => {
    ...
    const task = {
      ...
      deadline:
        taskType === "shortTerm" ? new Date().toLocaleDateString() : null, // 단기 작업의 마감일 설정
    };
  ...
  const deleteSelectedTasks = () => {
    // 선택된 집안일 제거
    const newDailyTasks = dailyTasks.filter(
      (_, index) => !selectedTasks.has(index)
    );
    const newShortTermTasks = shortTermTasks.filter(
      (_, index) => !selectedTasks.has(index)
    );
              ...
              {dailyTasks.map((task, index) => (
                <li
                  ...
                  <input
                    ...
                    onChange={() =>
                      handleTaskSelect(index, dailyTasks, setDailyTasks)
                    }
                    className={styles.checkBox}
                  />
                  <h4>
                    {task.assignedTo} : {task.name} - {task.points} 포인트
                  </h4>
                  <input
                    type='checkbox'
                    checked={task.completed}
                    onChange={() =>
                      toggleTaskCompletion(dailyTasks, setDailyTasks, index)
                    }
                    className={styles.checkBox}
                  />
                </li>
              ...
              {shortTermTasks.map((task, index) => (
                <li
                  ...
                  <input
                    ...
                    checked={selectedTasks.has(index)}
                    onChange={() =>
                      handleTaskSelect(index, shortTermTasks, setShortTermTasks)
                    }
                    className={styles.checkBox}
                  />
                  <h4>
                    마감일: {task.deadline} - {task.name} - {task.points} 포인트
                  </h4>
                  <input
                    type='checkbox'
                    checked={task.completed}
